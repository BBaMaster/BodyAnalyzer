                    ...
                    if ((IR_AC_Signal_Previous < 0 && IR_AC_Signal_Current >= 0) || 
                        ...
                        if ((IR_AC_Max - IR_AC_Min) > 20 && (IR_AC_Max - IR_AC_Min) < 1000) {
                            ...
                            if (elapsed_time > DEBOUNCE_TIME_MS) { // Debounce period
                                ...
                                OSQPost(&CommQI2C, &average_heart_rate, sizeof(average_heart_rate), OS_OPT_POST_FIFO, &os_err);
                                if (os_err == OS_ERR_Q_MAX){
                                  Log_Write(LOG_LEVEL_ERROR, "I2C Task: queue is full can't send the value to task...", 0);
                                } else if (os_err != OS_ERR_NONE) {
                                  Log_Write(LOG_LEVEL_ERROR, "Error occured at I2C Task...", 0);
                                } else {
                                  Log_Write(LOG_LEVEL_I2C, "Sent value to processing task...", 0);
                                }
